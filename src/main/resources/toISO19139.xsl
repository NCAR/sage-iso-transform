<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  xmlns:gml="http://www.opengis.net/gml" xmlns:cat="http://standards.iso.org/iso/19115/-3/cat/1.0"
  ...
  <xd:doc scope="stylesheet">
    <xd:desc>
      <xd:p> More work required on: <xd:ul>
  ...
  <xsl:template match="mdb:metadataStandard" priority="5">
    ...
    <xsl:for-each
      select="//mdb:identificationInfo/mri:MD_DataIdentification/mri:citation/cit:CI_Citation/cit:onlineResource/cit:CI_OnlineResource">
        ...
        <xsl:element name="{concat($nameSpacePrefix,':',local-name(.))}">
          ...
          <xsl:call-template name="writeCodelistElement">
            ...
            <xsl:with-param name="codeListValue"
              select="mri:status/mri:MD_ProgressCode/@codeListValue"/>
          ...
          <xsl:for-each
            select="
              mri:defaultLocale/lan:PT_Locale/lan:characterEncoding |
          ...
          <xsl:call-template name="writeCodelistElement">
            ...
            <xsl:with-param name="codeListValue"
              select="srv2:couplingType/srv2:SV_CouplingType/@codeListValue"/>
          </xsl:call-template>
  ...
  <xsl:template match="mdb:distributionInfo">
    ...
    <xsl:if test="$mergeAllOnlineResourcesInDistribution">
      ...
      <xsl:variable name="hasRelation"
        select="
          count(ancestor::mdb:MD_Metadata/descendant::*[
          ...
          <gmd:MD_Distribution>
            <gmd:transferOptions>
              <gmd:MD_DigitalTransferOptions>
                <xsl:for-each
                  select="
                    ancestor::mdb:MD_Metadata/descendant::*[
                    local-name() = $functionMap/entry/@key
        ...
        <xsl:for-each select="mdq:DQ_DataQuality/mdq:report/*">
          <gmd:report>
            <xsl:element name="{concat('gmd:',local-name())}">
              ...
              <xsl:apply-templates
                select="mdq:measure/mdq:DQ_MeasureReference/mdq:measureIdentification"/>
              <xsl:call-template name="writeCharacterStringElement">
              ...
              <gmd:dateTime>
                <xsl:apply-templates
                  select="mdq:evaluation/mdq:DQ_FullInspection/mdq:dateTime/gco2:DateTime"/>
              </gmd:dateTime>
  ...
  <xsl:template match="cit:CI_Responsibility" priority="5">
    ...
    <xsl:choose>
      <!--
          CI_ResponsibleParties that include gcx:Anchor elements (individualName, organisationName, or positionName) are also translated to CI_Responsibilities.
        -->
      <xsl:when
        test="
  ...
  <xsl:template match="cit:party/*/cit:contactInfo/cit:CI_Contact/cit:phone">
    ...
    <xsl:variable name="phoneType"
      select="
        if (cit:numberType/cit:CI_TelephoneTypeCode/@codeListValue != '')
        then
          cit:numberType/cit:CI_TelephoneTypeCode/@codeListValue
        else
          cit:numberType/cit:CI_TelephoneTypeCode"/>
    <xsl:if
      test="count(preceding-sibling::node()[cit:numberType/cit:CI_TelephoneTypeCode = $phoneType]) = 0">
  ...
  <xsl:template match="mrd:MD_Format">
    <xsl:element name="gmd:MD_Format">
      ...
      <xsl:if test="mrd:formatSpecificationCitation/cit:CI_Citation/cit:alternateTitle">
        <gmd:specification>
          <xsl:apply-templates
            select="mrd:formatSpecificationCitation/cit:CI_Citation/cit:alternateTitle/*"/>
        </gmd:specification>
      </xsl:if>
  ...
  <xsl:template name="getNamespacePrefix">
    ...
    <xsl:variable name="prefix">
      <xsl:choose>
        ...
        <xsl:when
          test="
            ancestor-or-self::mco:MD_Constraints
        ...
        <xsl:when
          test="ancestor-or-self::cit:CI_ResponsibleParty or ancestor-or-self::cit:CI_OnlineResource">
          <xsl:text>gmd</xsl:text>
        </xsl:when>
        <xsl:when
          test="
            ancestor-or-self::mcc:MD_ScopeCode or ancestor-or-self::mcc:MX_ScopeCode
        ...
        <xsl:when
          test="
            ancestor-or-self::msr:MD_Georectified or ancestor-or-self::msr:MI_Georectified
        ...
        <xsl:when
          test="
            ancestor-or-self::mri:MD_DataIdentification
        ...
        <xsl:when
          test="ancestor-or-self::mrc:MD_CoverageDescription or ancestor-or-self::mrc:MI_CoverageDescription">
          <xsl:text>gmd</xsl:text>
        </xsl:when>
